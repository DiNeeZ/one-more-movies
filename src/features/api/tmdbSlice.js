...
export const tmdbSlice = createApi({
  ...
  endpoints: (builder) => ({
    getMovie: builder.query({
      ...
      transformResponse: (res, _, { __, mediaType }) =>
        transformer.transformMovie(res, mediaType),
    }),
    ...
    getSimilar: builder.query({
      ...
      transformResponse: (res, _, { __, mediaType }) =>
        transformer.transformSimilar(res, mediaType),
    }),
    ...
    getPersonCredits: builder.query({
      query: (id) =>
        `person/${id}?api_key=${API_KEY}&language=en-US&append_to_response=combined_credits`,
      transformResponse: (res) => transformer.transformPersonCredits(res),
    }),
    getSearchResults: builder.query({
      query: ({ searchQuery, page }) => {
        console.log(searchQuery);
        return `search/multi?api_key=${API_KEY}&query=${searchQuery}&language=en-US&page=${page}&include_adult=false`;
      },
